...
const LICENSE_ENDPOINTS = [
    'https://script.google.com/macros/s/AKfycbzT2sTUOnnh_aOIcAKbmSKlkErydYm0zfgT7cKXPINOa8Dl5TD55ttydSsmmUlc--zZSw/exec'
];
        ...
        try {
            ...
            try {
                if (responseText.trim().startsWith('<')) {
                    // È probabile che sia un errore HTML di Google (es. "Too many scripts")
                    const errorMsg = responseText.includes('body')
                        ? responseText.match(/<body[^>]*>([\s\S]*?)<\/body>/i)[1].replace(/<[^>]*>/g, '').trim()
                        : responseText.substring(0, 100);

                    console.error(`[Endpoint ${endpointNum}] HTML Error received:`, errorMsg);
                    throw new Error('Received HTML instead of JSON: ' + errorMsg);
                }
